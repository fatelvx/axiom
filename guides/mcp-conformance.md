...
| `axiom_infer_contract` | Treat inference as current-graph authoring evidence. It is not declared architecture intent and should not be saved as `.axi` without human review. Show `starterContract.reviewPass[]` before suggesting that a user promote the draft. For infer payloads, prefer `summary.observedModuleEdges` for starter-contract dependency edges and `summary.observedImportSites` for the import-site evidence behind them; `summary.observedDependencies` is only a compatibility alias for module-edge count. |
| `axiom_observe_inferred_contract` | Treat this as advisory review evidence produced from a temporary inferred contract. It can help no-contract projects get warning context, but it is not a gate and not declared intent. Read the nested `inference.starterContract.reviewPass[]` and `inference.summary` counts with the same review-pass and module-edge versus import-site distinctions as `axiom_infer_contract`. |
