...
export function formatGraphMermaid(result: CheckResult, options: GraphFormatOptions = {}): string {
  ...
  const edges = aggregateMermaidEdges(graph);
  const lines = [
    `%% Generated by ${options.observe ? "axi observe" : "axi graph"} --mermaid.`,
...
function aggregateMermaidEdges(graph: GraphJsonResult): MermaidEdge[] {
